...
frappe.ui.form.on('Courses Scheduling Tool', {
	setup: function(frm) {
	...
	assessment_plan: function(frm) {
		...
		if(frm.doc.assessment_plan) {
			if (!frm.doc.student_group)
				return
			frappe.call({
				method: "education.education_contoral.doctype.courses_scheduling_tool.courses_scheduling_tool.get_assessment_students",
				args: {
	...
	render_table: function(frm) {
		...
		frappe.call({
			method: "education.education_contoral.doctype.courses_scheduling_tool.courses_scheduling_tool.get_assessment_details",
			args: {
	...
	get_marks: function(frm, criteria_list) {
		...
		var result_table = $(frappe.render_template('courses_schedule', {
			frm: frm,
		...
		result_table.on('change', 'input', function(e) {
			...
			if (Object.keys(student_scores["assessment_details"]).length === criteria_list.length) {
				...
				frappe.call({
					method: "education.education_contoral.doctype.courses_scheduling_tool.courses_scheduling_tool.mark_assessment_result",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
	...
	student_group:function(frm){
		// from assessment_plan function
		frm.doc.show_submit = false;
		if(frm.doc.assessment_plan) {
			if (!frm.doc.student_group)
				return
			frappe.call({
				method: "education.education_contoral.doctype.courses_scheduling_tool.courses_scheduling_tool.get_course_schedules",
				args: {
					"student_group": frm.doc.student_group
				},
				callback: function(r) {
					if (r.message) {
						frm.doc.course_schedules = r.message;
	...
	submit_result: function(frm) {
		if (frm.doc.show_submit) {
			frm.page.set_primary_action(__("Submit"), function() {
				frappe.call({
					method: "education.education_contoral.doctype.courses_scheduling_tool.courses_scheduling_tool.submit_assessment_results",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
						"student_group": frm.doc.student_group
					},
